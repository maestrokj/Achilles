...
3. **Wireframes nav** — links to the `_wireframes/` screens; this nav **is** the
   ...
   (`.wz-tag`): the common kind is `.wz-tag--screen` (“screen”); a rarer kind
...
- **Load** — the screen has a loading / waiting state; links to the matching
  ...
  `<a class="in-load in-load--<mech>" href="…/shared-ui/_wireframes/loading-states.html#<anchor>"><span class="in-load__icon"></span>loading</a>`.
...
(`⋯ actions menu · Owner`), but the reader hovers the trigger to see the open
...
<span class="wf-guarded"
  ><a class="wf-a wf-a--danger">Delete</a
  ><a
    class="wf-guard"
    href="../../shared-ui/_wireframes/confirm-dialog.html#irreversible"
    title="Action guarded by confirmation"
...
```html
<span class="ai-tag" title="Step runs a neural network — model inference"
